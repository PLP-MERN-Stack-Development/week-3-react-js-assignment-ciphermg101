          ...
          {error && (
            ...
            >
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium">{error || t('posts.error')}</p>
                </div>
              </div>
            </motion.div>
          )}
                ...
                >
                  ...
                  <Card.Footer className="flex justify-between items-center border-t border-border/50">
                    <span className="text-xs text-muted-foreground">
                      {t('posts.minRead', { minutes: Math.ceil(post.body.length / 100) })}
                    </span>
                    <Link 
                      to={`/posts/${post.id}`}
          ...
          >
            <div className="max-w-md mx-auto">
              ...
              <p className="mt-2 text-muted-foreground">
                {searchQuery 
                  ? t('posts.noPosts.noSearchResults', { query: searchQuery })
                  : t('posts.noPosts.noPostsAvailable')}
              </p>
              {searchQuery && (
                <Button 
                  variant="outline" 
          ...
          >
            <nav className="flex items-center gap-2 bg-background/50 backdrop-blur-sm p-2 rounded-full border border-border/50 shadow-sm" aria-label="Pagination">
              <Button
                ...
                aria-label={t('posts.pagination.first')}
              >
                &laquo;
              </Button>
              <Button
                ...
                disabled={pagination.page === 1}
                className="rounded-full w-10 h-10 p-0"
                aria-label={t('posts.pagination.previous')}
              >
                &lsaquo;
              </Button>
              ...
              <div className="flex items-center px-2">
                <span className="text-sm text-foreground/80">
                  {t('posts.pagination.page')} <span className="font-medium">{pagination.page}</span> {t('posts.pagination.of')} <span className="font-medium">{pagination.totalPages}</span>
                </span>
              </div>
              ...
              <Button
                ...
                className="rounded-full w-10 h-10 p-0"
                aria-label={t('posts.pagination.next')}
              >
                &rsaquo;
              </Button>
              <Button
                ...
                disabled={pagination.page === pagination.totalPages}
                className="rounded-full w-10 h-10 p-0"
                aria-label={t('posts.pagination.last')}
              >
                &raquo;
              </Button>
            </nav>
